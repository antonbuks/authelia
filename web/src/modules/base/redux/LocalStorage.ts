...
class LocalStorage extends EventEmitter {
     ...
     */
    setLocalStorageDisabled(value: boolean) {
     ...
     */
    setItem(keyName: string, keyValue: string, dontEmitChangedEvent = false) {
    ...
    serialize(ignore = []) {
        ...
        for (let i = 0; i < length; i++) {
            ...
            if (!ignore.includes(key as never)) {
                // @ts-ignore
...
export const autheliaLocalStorage = new LocalStorage();
