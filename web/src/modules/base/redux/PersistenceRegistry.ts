...
class PersistenceRegistry {
    ...
    persistState(state: Object) {
        ...
        if (checksum !== this._checksum) {
            for (const subtreeName of Object.keys(filteredState)) {
                try {
                    autheliaLocalStorage.setItem(subtreeName, JSON.stringify(filteredState[subtreeName]));
                } catch (error) {
    ...
    _getPersistedSubtree(subtreeName: string, subtreeConfig: Object, subtreeDefaults?: Object) {
        let persistedSubtree = autheliaLocalStorage.getItem(subtreeName);
        ...
        if (persistedSubtree) {
            try {
                ...
                console.error("Error parsing persisted subtree", subtreeName, persistedSubtree, error);
            }
        }
